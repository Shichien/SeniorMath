...
% \begin{example}
%     在 $\解三角形 ABC$ 中，已知 $\sin A = 2\sin B \cdot \sin C$，求证：$\解三角形 ABC$ 是直角三角形。
% \end{example}

% \begin{proof}
%     在 $\解三角形 ABC$ 中，由于 $A+B+C=\pi$，所以 $\sin A = \sin(\pi-A) = \sin(B+C)$。

%     由题意条件 $\sin A = 2\sin B \cdot \sin C$，我们有：
%     $$\sin(B+C) = 2\sin B \cdot \sin C$$

%     利用正弦和角公式：$\sin(B+C) = \sin B \cos C + \cos B \sin C$

%     所以：
%     $$\sin B \cos C + \cos B \sin C = 2\sin B \cdot \sin C$$

%     整理得：
%     $$\sin B \cos C = \sin B \cdot \sin C$$

%     因为在三角形中 $\sin B \neq 0$，所以：
%     $$\cos C = \sin C$$

%     这意味着 $C = \frac{\pi}{4}$ 或 $C = \frac{5\pi}{4}$。由于 $C$ 是三角形的内角，$0 < C < \pi$，所以 $C = \frac{\pi}{4}$。

%     此时 $\cos C = \sin C = \frac{\sqrt{2}}{2}$，代入 $\cos C = \sin C$ 得 $\cos C = \frac{1}{\sqrt{2}}$，即 $\cos C = \frac{\sqrt{2}}{2}$。

%     事实上，我们可以直接使用 $\sin 2C = 2\sin C \cos C$，当 $\sin C = \cos C$ 时，有 $\sin 2C = 2\sin^2 C$。

%     因为 $A+B+C=\pi$ 且 $C = \frac{\pi}{4}$，所以 $A+B = \frac{3\pi}{4}$。

%     从 $\sin A = \sin(B+C) = \sin(B+\frac{\pi}{4})$ 可知，$A = B + \frac{\pi}{4}$ 或 $A = \pi - (B+\frac{\pi}{4})$。

%     考虑第二种情况：$A = \pi - B - \frac{\pi}{4} = \frac{3\pi}{4} - B$

%     又因为 $A+B = \frac{3\pi}{4}$，所以 $B + \frac{3\pi}{4} - B = \frac{3\pi}{4}$，成立。

%     所以 $A = \frac{3\pi}{4} - B$，$B + A = \frac{3\pi}{4}$，$C = \frac{\pi}{4}$。

%     由此可知 $A + B + C = \pi$，且 $B + C = \frac{\pi}{2}$，即角 $B$ 和角 $C$ 互补。

%     在三角形中，如果两个角互补，则它们夹着的角是直角。因此，$\angle A = 90°$，$\解三角形 ABC$ 是直角三角形。
% \end{proof}

% \begin{tips}
%     这个例子展示了如何使用 $\sin A = \sin(B+C)$ 这一重要关系来解决三角形问题。在这里，我们利用该关系结合正弦和角公式，推导出三角形的特殊性质。
% \end{tips}
\subsection{边角互化}
\begin{example}
...
\begin{example}
    ...
    \begin{itemize}
        \item 若 $a=2 \sqrt{3}, ~ A=\frac{\pi}{3}$ ，则 $\triangle A B C$ 的外接圆的面积为 $16 \pi$
        \item 已知 $(a+b+c)(a+b-c)=(2+\sqrt{3}) a b$ ，则 $C=\frac{\pi}{3}$
        \item 若 $\sin ^2 A>\sin ^2 B+\sin ^2 C$ ，则 $\triangle A B C$ 为钝角三角形
        \item 若 $\triangle A B C$ 为锐角三角形，则 $\sin A>\cos B$
    \end{itemize}
\end{example}
