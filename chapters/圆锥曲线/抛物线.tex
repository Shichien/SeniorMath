...
\begin{definition}
    抛物线是平面内到定点（焦点）和定直线（准线）距离相等的点的集合。
\end{definition}

\begin{example}
    求平面上一点到点 $(2,0)$ 和到直线 $x=-2$ 的距离相等的点的轨迹方程。
\end{example}
...
\subsection{抛物线和直线联立的弦长}

\begin{example}
    已知抛物线 $C: y^2=2 p x$ 的焦点为 $F, ~ M(1, t)$ 为抛物线 $C$ 上的点，且 $|M F|=\frac{3}{2}$ ．
    \begin{enumerate}
        \item 求抛物线 $C$ 的方程；
        \item 若直线 $y=x-2$ 与抛物线 $C$ 相交于 $A, B$ 两点，求弦长 $|A B|$ ．
    \end{enumerate}
\end{example}
...
\begin{example}
    已知抛物线 $C$ 顶点在原点，焦点在 $x$ 轴上，且经过点 $(5,-2 \sqrt{5})$ ，一条斜率为 $\sqrt{3}$ 的直线过抛物线 $C$ 的焦点，且与 $C$ 交于 $A$ ， $B$ 两点，
    \begin{enumerate}
        \item 求抛物线方程；
        \item 求弦 $|A B|$ 的长度；
    \end{enumerate}
...
\begin{theorem}
    ...
    $$|A B|=|A F|+|B F|=\frac{2 p}{1-\cos ^2 \alpha}=\frac{2 p}{\sin ^2 \alpha}$$
    另外，这两条焦半径的长度还可以得到倒数关系：
...
根据 $y_1 y_2 = - p^2$ 和 $x_1 x_2 = \frac{p^2}{4}$，可以得到一些特殊关系。

\begin{theorem}[斜率和点乘定值]
    \begin{enumerate}
        \item $k_{OA} \times k_{OB} = -4$
        \item $\overrightarrow{OA} \cdot \overrightarrow{OB} = -\frac34p^2$
    \end{enumerate}
\end{theorem}

\begin{proof}
    $\overrightarrow{OA} \cdot \overrightarrow{OB} = x_1 x_2 + y_1 y_2 = -\frac34p^2$
\end{proof}

\begin{proof}
    $k_{OA} \times k_{OB} = \frac{y_1}{x_1} \times \frac{y_2}{x_2} = \frac{y_1 y_2}{x_1 x_2} = \frac{-p^2}{\frac{p^2}{4}} = -4$
\end{proof}