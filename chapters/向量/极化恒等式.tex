...
\begin{example}
    如图，$\triangle ABC$是边长为$2\sqrt{3}$的正三角形，$P$是以$C$为圆心，半径为$1$的圆上任意一点，则$\overrightarrow{PA} \cdot \overrightarrow{PB}$的取值可能是（\quad）
    \begin{tasks}(4)
        \task 0
        \task 1
        \task 6
...
\begin{example}
    如图，在$\triangle ABC$中，$AB \perp AC, \angle ABC = 30^\circ, AC=1, D$是$BC$的中点，$E$是以$B$为圆心，$BD$为半径的圆上任意一点，则$\overrightarrow{AD} \cdot \overrightarrow{AE}$的值可能为（\quad）
    \begin{center}
...
\begin{example}
    如图，已知正方形$ABCD$的边长为2，若动点$P$在以$AB$为直径的半圆$E$（正方形$ABCD$内部，含边界）上，则$\overrightarrow{PC} \cdot \overrightarrow{PD}$的取值范围为 \underline{\qquad}.
    \begin{center}
...
\begin{example}
    如图，边长为2的菱形$ABCD$的对角线相交于点$O$，点$P$在线段$BO$上运动，若$\overrightarrow{AB} \cdot \overrightarrow{AO}=1$，则$\overrightarrow{PA} \cdot \overrightarrow{PB}$的最小值为 \underline{\qquad}.
    \begin{center}
        \begin{tikzpicture}[scale=1.2]
...
\begin{example}
    已知正$\triangle ABC$的边长为2，点$P$为$\triangle ABC$所在平面内的动点，且$PC=1$，则$\overrightarrow{PA} \cdot \overrightarrow{PB}$的取值范围为 \underline{\qquad}.
\end{example}
