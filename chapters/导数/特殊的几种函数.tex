...
\subsubsection{三次函数的对称中心}

\begin{example}
    已知函数 $f(x)=x^3-x$ ，曲线 $f(x)$ 经过点 $P(1,0)$ 的切线方程为 \underline{\qquad}.
\end{example}

\v{8em}

\subsubsection{三次方程的因式分解}

\begin{example}
    已知函数 $f(x)=x^3-3 x^2+1$，下列选项正确的是（ \quad ）
    \begin{tasks}(2)
        \task $f(x)$ 在 $(0,2)$ 上单调递增
        \task $y=f(x+1)+1$ 是奇函数
        \task 过点 $(0,1)$ 可作曲线 $y=f(x)$ 的两条切线
        \task 当 $a \geq 2 \sqrt{3}$ 时，$f(x+a) \geq f(x)$ 恒成立
    \end{tasks}
\end{example}
